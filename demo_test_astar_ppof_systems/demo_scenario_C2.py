#!/usr/bin/env python3
"""
Scenario C2: Direct Path Between C1 and C3
==========================================

This scenario demonstrates direct pathfinding between two specific coordinates:
- C1: (176.553, 6.028, 150.340) - Same origin as C1
- C3: (174.860, 15.369, 136.587) - New destination
...
def run_scenario_C2():
    ...
    """
    print("🚀 Scenario C2: Direct Path Between C1 and C3")
    print("=" * 60)
    print()
    
    # Scenario coordinates
    origin = (176.553, 6.028, 150.340)      # C1 - Same as previous
    destination = (174.860, 15.369, 136.587)  # C3 - New destination
    ...
    # ====================================================================
    print("🧪 Test C2.1: Endpoint System Analysis")
    print("-" * 50)
    ...
    try:
        ...
        if not destination_found:
            print(f"❌ Destination C3 not found in graph - cannot proceed")
            return None
    ...
    # ====================================================================
    print("🧪 Test C2.2: Direct Distance Calculation")
    print("-" * 50)
    ...
    try:
        ...
        # Note the significant elevation drop
    ...
    # ====================================================================
    print("🧪 Test C2.3: Optimal Path Finding")
    print("-" * 50)
    ...
    try:
        ...
        if len(path) > 0:
            ...
            print(f"     Start: {start_z:.3f}")
            print(f"     End: {end_z:.3f}")
            print(f"     Range: {min_z:.3f} to {max_z:.3f}")
            print(f"     Total Drop: {start_z - end_z:.3f} units")
        
        # Store results for DXF export
        scenario_results = {
            "scenario": "C2",
            "title": "Direct Path Between C1 and C3",
            "origin": {"name": "C1", "coord": origin, "system": origin_system},
            "destination": {"name": "C3", "coord": destination, "system": destination_system},
...
def create_scenario_C2_summary(results):
    """Create a summary of Scenario C2 results for documentation."""
    if not results:
        return None
    ...
    summary = {
        "scenario": "C2",
        "title": "Direct Path Between C1 and C3",
        "description": "Optimal pathfinding between C1 and C3 coordinates with significant elevation change",
        "coordinates": {
            "origin": {"point": "C1", "coord": results["origin"]["coord"], "system": results["origin"]["system"]},
            "destination": {"point": "C3", "coord": results["destination"]["coord"], "system": results["destination"]["system"]}
        },
...
if __name__ == "__main__":
    print("Scenario C2: Direct Path Between C1 and C3")
    print("Finding optimal path with elevation analysis")
    print()
    
    results = run_scenario_C2()
    
    if results:
        print("\n🎉 Scenario C2 completed successfully!")
        ...
        print(f"✅ Elevation change: {results['metrics']['elevation_change']:.3f} units")
        print(f"✅ Ready for DXF export")
        
        # Create summary for documentation
        summary = create_scenario_C2_summary(results)
        print(f"\n📄 Summary created for documentation")
        
        # Save results for DXF export
        results_file = "scenario_C2_results.json"
        try:
