...
class OptimizedSpatialGraph3D:
    ...
    # ───────────────────  NUEVA LÓGICA "EDGE-SPLIT"  ────────────────────
    # 1)  p   sobre segmento [a-b]  (funciona en 3D)
    def _point_on_segment(self,
                          p: tuple,
                          a: tuple,
                          b: tuple,
                          tol: float) -> bool:
        """
        Devuelve True si el punto p cae sobre el segmento (a-b)
        con precisión ± tol (mismas unidades que tus coords).
        """
        # Vector AB y proyección ortogonal de AP sobre AB
        ab = (b[0]-a[0], b[1]-a[1], b[2]-a[2])
        ap = (p[0]-a[0], p[1]-a[1], p[2]-a[2])

        ab_len2 = ab[0]**2 + ab[1]**2 + ab[2]**2
        if ab_len2 == 0:                       # arista degenerada
            return False

        # Par. de la proyección (0 = a, 1 = b)
        t = (ab[0]*ap[0] + ab[1]*ap[1] + ab[2]*ap[2]) / ab_len2
        if t < -1e-12 or t > 1+1e-12:          # fuera del tramo
            return False

        # Punto más cercano y distancia real
        closest = (a[0] + t*ab[0], a[1] + t*ab[1], a[2] + t*ab[2])
        return self.euclidean_distance(p, closest) <= tol


    # 2)  Corta la arista (u,v) y crea nodo virtual p
    def _split_edge(self,
                    u: tuple,
                    v: tuple,
                    p: tuple) -> tuple:
        """
        Elimina (u,v) y la sustituye por (u,p) + (p,v).
        Actualiza self.graph   **y**  self.graph_data.
        """
        if not self.graph.has_edge(u, v):
            return None

        # a) Eliminar la arista original (una sola vez: grafo no dirigido)
        self.graph.remove_edge(u, v)
        if self.graph_data is not None:
            self.graph_data[u] = [n for n in self.graph_data[u] if n != v]
            self.graph_data[v] = [n for n in self.graph_data[v] if n != u]

        # b) Añadir nodo virtual (si no existía)
        self.graph.add_node(p)
        if self.graph_data is not None and p not in self.graph_data:
            self.graph_data[p] = []

        # c) Crear los dos nuevos tramos con pesos correctos
        d_up = self.euclidean_distance(u, p)
        d_pv = self.euclidean_distance(p, v)

        self.graph.add_edge(u, p, weight=d_up)
        self.graph.add_edge(p, v, weight=d_pv)

        if self.graph_data is not None:
            self.graph_data[u].append(p)
            self.graph_data[v].append(p)
            self.graph_data[p].extend((u, v))

        return p


    # 3)  Garantiza que q sea nodo (lo crea si está en una arista)
    def _ensure_node(self,
                     q: tuple,
                     tol: float) -> Union[tuple, None]:
        """
        - q YA es nodo    → lo devuelve intacto  
        - q sobre arista  → crea nodo virtual y lo devuelve  
        - en otro sitio   → None
        """
        if q in self.graph:
            return q

        # Scan rápido de todas las aristas (una cada vez: undirected)
        for u, v, _ in self.graph.edges(data=True):
            if self._point_on_segment(q, u, v, tol):
                return self._split_edge(u, v, q)
        return None


    # 4)  Algoritmo público: usa split-edge SOLO si hace falta
    def find_path_with_edge_split(self,
                                  start_point: tuple,
                                  goal_point: tuple,
                                  tol: Optional[float] = None):
        """
        A* que acepta puntos intermedios sobre aristas:
          • Si ambos puntos existen como nodos  → usa A* puro
          • Si alguno está en una arista       → corta la arista y sigue
          • Si no hay forma de encajar         → (None, 0)
        """
        tol = tol or self.tolerance

        # 4.1  Garantizar nodos
        s = self._ensure_node(start_point, tol)
        g = self._ensure_node(goal_point,  tol)

        # 4.2  Abortamos si alguno quedó inalcanzable
        if s is None or g is None:
            return None, 0

        # 4.3  Todo listo → A*
        return self.astar_path_with_visited(s, g)
    # ─────────────────────────────────────────────────────────────────────
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced A* pathfinding with tolerance-based coordinate matching and edge split for intermediate points",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage with automatic testing (default)
  python3 astar_spatial_IP.py graph.json 152.3 17.9 160.1 139.2 28.2 140.0
  
  # Enable edge split for intermediate points on edges
  python3 astar_spatial_IP.py graph.json 152.3 17.9 160.1 139.2 28.2 140.0 --edge-split
  
  # Custom tolerance with edge split
  python3 astar_spatial_IP.py graph.json 152.3 17.9 160.1 139.2 28.2 140.0 --tolerance 0.5 --edge-split
  
  # Skip tests for production use
  python3 astar_spatial_IP.py graph.json 152.3 17.9 160.1 139.2 28.2 140.0 --skip-tests
  
  # Strict tolerance for exact matches only
  python3 astar_spatial_IP.py graph.json 152.3 17.9 160.1 139.2 28.2 140.0 --tolerance 0.01

Edge Split Mode:
  --edge-split allows coordinates that fall on edges between nodes. The algorithm
  will automatically split the edge and create virtual intermediate nodes.
...
Note: Tests are automatically run by default to ensure algorithm robustness.
    ...
    print(f"Enhanced A* Pathfinding Algorithm with Intermediate Points (IP)")
    print(f"Graph file: {args.graph_file}")
    print(f"Tolerance: {args.tolerance} units")
    print(f"Grid size: {args.grid_size} units")
    print(f"Edge split: {'Enabled' if args.edge_split else 'Disabled'}")
    ...
    # Execute pathfinding with tolerance and optional edge split
    print(f"\nSearching for path:")
    print(f"  Start: {format_point(start_point)}")
    print(f"  Goal:  {format_point(goal_point)}")
    
    if args.edge_split:
        print(f"  Mode: Edge Split Enabled (intermediate points on edges allowed)")
        # Use edge split pathfinding
        path, nodes_explored = graph.find_path_with_edge_split(start_point, goal_point)
        graph.nodes_explored = nodes_explored
        # Create match_info for compatibility with existing display code
        if path is not None:
            match_info = {
                'start_query': start_point,
                'goal_query': goal_point,
                'start_match': start_point,  # In edge split mode, we use the exact coordinates
                'goal_match': goal_point,
                'start_distance': 0.0,       # Edge split creates exact matches
                'goal_distance': 0.0,
                'start_usable': True,
                'goal_usable': True,
                'both_usable': True,
                'tolerance_used': graph.tolerance
            }
        else:
            match_info = {
                'start_query': start_point,
        ...
        print(f"  Mode: Standard (exact node matching with tolerance)")
    ...
    if not match_info['both_usable']:
        ...
        if args.edge_split:
            # Edge split mode - simpler error reporting
            print(f"Edge split mode could not find or create valid nodes for:")
            if not match_info['start_usable']:
                print(f"  Start: {format_point(match_info['start_query'])}")
            if not match_info['goal_usable']:
                print(f"  Goal: {format_point(match_info['goal_query'])}")
            print(f"\nThe coordinates are not on existing nodes or edges within tolerance {args.tolerance}")
        else:
            # Standard mode - detailed error reporting with MatchResult objects
            if not match_info['start_usable']:
        ...
        print(f"\nRecommendations:")
        if not args.edge_split:
            if match_info['start_match'] and match_info['goal_match']:
            ...
            print(f"  2. Try edge split mode: --edge-split")
            print(f"  3. Use coordinates closer to existing graph nodes")
            print(f"  4. Check if your coordinates are in the correct coordinate system")
        else:
            print(f"  1. Increase tolerance: --tolerance {args.tolerance * 2}")
            print(f"  2. Check if your coordinates are in the correct coordinate system")
            print(f"  3. Verify graph coverage in target area")
        print("="*50)
